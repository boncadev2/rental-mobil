...
export default function Show({booking, invoice, payments, checkout, checkin}) {
    ...
    return <AuthenticatedLayout header={booking.booking_code}>
        ...
        <div className="mx-auto grid max-w-5xl gap-5 p-6 md:grid-cols-2">
            ...
            <section className="rounded-2xl bg-white p-6 shadow"><h3 className="font-bold">Kendaraan</h3><p className="mt-3">{booking.vehicle.brand} {booking.vehicle.model}</p><p>Status: {booking.status}</p>{checkoutOk && <Link href={route('admin.checkouts.create', booking.id)} className="mt-5 inline-block rounded-xl bg-indigo-600 px-4 py-3 font-bold text-white">Proses checkout</Link>}{booking.status === 'RETURN_INSPECTION' && <Link href={route('admin.returns.create', booking.id)} className="mt-5 inline-block rounded-xl bg-amber-500 px-4 py-3 font-bold text-white">Proses check-in pengembalian</Link>}</section>
            <section className="rounded-2xl bg-white p-6 shadow"><h3 className="font-bold">Invoice</h3>{invoice ? <div className="mt-3 space-y-2 text-sm"><p className="flex justify-between"><span>Total booking</span><b>{money(invoice.total)}</b></p><p className="flex justify-between"><span>Sudah dibayar</span><b className="text-emerald-700">{money(invoice.paid_amount)}</b></p><p className="flex justify-between border-t pt-2"><span>Sisa hutang</span><b className="text-amber-700">{money(invoice.balance)}</b></p>{Number(invoice.balance) > 0 && <button type="button" onClick={() => setCashModalOpen(true)} className="mt-3 w-full rounded-xl bg-emerald-600 px-4 py-3 font-bold text-white">Catat pelunasan cash · {money(invoice.balance)}</button>}<p className="text-xs font-bold text-indigo-600">{invoice.status}</p></div> : <p className="mt-3">Belum dibuat</p>}</section>
